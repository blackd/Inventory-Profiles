...
//    rClearDepth() // use translate or zOffset
    ...
    private fun drawHotSprite() {
        ...
        rDisableDepth()
        RenderSystem.enableBlend()
        val screenWidth = Vanilla.mc().window.guiScaledWidth
        val screenHeight = Vanilla.mc().window.guiScaledHeight
        val i = screenWidth / 2;
        for (j1 in 0..8) {
            if (j1 in lockedInvSlotsStoredValue) {
                val lockedSprite: Sprite = if (j1 - Vanilla.playerInventory().selected == -1) {
                    hotOutlineLeft
                } else if (j1 - Vanilla.playerInventory().selected == 1) {
                    hotOutlineRight
                } else if (j1 == Vanilla.playerInventory().selected) {
                    hotOutlineActive
                } else {
                    hotOutline
                }
