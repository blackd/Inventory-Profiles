...
public class ForgeEventHandler {
    ...
    public void onGuiKeyPressedPre(GuiScreenEvent.KeyboardKeyPressedEvent.Pre e) { // Tweaks.PREVENT_CLOSE_GUI_DROP_ITEM
        ...
        InputMappings.Input mouseKey = InputMappings.getKey(e.getKeyCode(), e.getScanCode()); //getInputByCode(e.getKeyCode(), e.getScanCode()); // getKey(e.getKeyCode(), e.getScanCode());
    ...
    //blockHitDelay
    Field blockHitDelayField = null; // destroyDelay

    //rightClickDelayTimer
    Field rightClickDelayTimerField = null; // rightClickDelay
    ...
    public void onTickPre() { // Tweaks.DISABLE_BLOCK_BREAKING_COOLDOWN, Tweaks.DISABLE_ITEM_USE_COOLDOWN
        ...
        if (Tweaks.INSTANCE.getDISABLE_BLOCK_BREAKING_COOLDOWN().getBooleanValue()) {
            if (pc == null || pc != Vanilla.INSTANCE.interactionManager()) {
                ...
                blockHitDelayField = ObfuscationReflectionHelper.findField(PlayerController.class, "destroyDelay");
            }
            try {
        ...
        if (Tweaks.INSTANCE.getDISABLE_ITEM_USE_COOLDOWN().getBooleanValue()) {
            if (rightClickDelayTimerField == null) {
                rightClickDelayTimerField = ObfuscationReflectionHelper.findField(Minecraft.class, "rightClickDelay");
            }
            try {
