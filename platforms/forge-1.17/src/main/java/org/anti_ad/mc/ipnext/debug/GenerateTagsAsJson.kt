...
object GenerateTagsAsJson: AbstractBlockScreenScriptGenerator() {
    ...
    override fun onClick(widget: ButtonWidget) {
        ...
        Vanilla.mc().execute() {
            ...
            ForgeRegistries.CONTAINERS.keys.forEach {
                if (it.namespace != "minecraft") {
                    namespaces.add(it.namespace)
                    val inBlocks = ForgeRegistries.BLOCKS.keys.contains(it)
                    val inItems = ForgeRegistries.ITEMS.keys.contains(it)
                    val inBlockEntities = ForgeRegistries.BLOCK_ENTITIES.keys.contains(it)
