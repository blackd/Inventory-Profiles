...
object NbtUtils {
    ...
    fun parseNbt(nbt: String): NbtCompound? {
        // StringNbtReader
        return tryCatch { StringNbtReader.parseCompoundFully(nbt)  }  //getTagFromJson(nbt) //parseTag // .parse()
    }

    fun parseNbtOrEmpty(nbt: String): NbtElement {
        return tryCatch {
            var z = StringNbtReader.create(NbtOps.INSTANCE).parseFully(StringReader(nbt))
            z
        } ?: NbtCompound()
    }
...
*/
    ...
    class WrappedTag(val value: NbtElement) {
        ...
        val asNumber: Number // todo what if number is long > double precision range
            get() = (value as? AbstractNbtNumber)?.asDouble()?.get()  ?: 0 //asDouble //double
        val asDouble: Double
            get() = (value as? AbstractNbtNumber)?.asDouble()?.get() ?: 0.0 //asDouble //double
