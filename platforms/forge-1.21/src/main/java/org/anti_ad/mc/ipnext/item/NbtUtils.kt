...
object NbtUtils {
    ...
    fun compareNbt(a: NbtCompound?,
        ...
        val keys1: List<String> = a.allKeys.sorted() // forge keySet() = allKeys
    ...
    fun parseNbt(nbt: String): NbtCompound? {
        // StringNbtReader
        return tryCatch { StringNbtReader.parseTag(nbt)  }  //getTagFromJson(nbt) //parseTag // .parse()
    }

    fun parseNbtOrEmpty(nbt: String): NbtElement {
        return tryCatch {
            var z = StringNbtReader(StringReader(nbt)).readValue()
            z
        } ?: NbtCompound()
    }
...
*/
    ...
    class WrappedTag(val value: NbtElement) {
        ...
        val asNumber: Number // todo what if number is long > double precision range
            get() = (value as? AbstractNbtNumber)?.asDouble  ?: 0 //asDouble //double
        val asDouble: Double
            get() = (value as? AbstractNbtNumber)?.asDouble ?: 0.0 //asDouble //double
    ...
    private fun innerMatchNbt(a: NbtCompound?,
        ...
        // NbtHelper.matches() //.compareNbt() //areNBTEquals
