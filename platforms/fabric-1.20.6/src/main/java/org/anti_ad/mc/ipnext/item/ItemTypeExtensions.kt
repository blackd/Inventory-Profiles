...
inline val ItemType.`(enchantmentsScore)`: Double
    get() = EnchantmentHelper.getEnchantments(vanillaStack).enchantmentsMap.fold(0.0) { acc, (enchantment, level) ->
...
inline val ItemType.`(enchantments)`: MutableMap<Enchantment, Int>
    get() = EnchantmentHelper.getEnchantments(vanillaStack).enchantmentsMap.associateTo (mutableMapOf<Enchantment, Int>()) {

        it.key.value() to it.intValue
    }
...
inline val ItemType.isStew: Boolean
    get() = this.`(isFood)` &&  (this.item is SuspiciousStewItem || this.item is StewItem) //vanillaStack.components[DataComponentTypes.FOOD]?.`(convertsToBawl)` ?: false
