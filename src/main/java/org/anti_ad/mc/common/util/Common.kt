...
inline fun <T> trySwallow(tryToRun: () -> T): T? = trySwallow(null,
                                                              tryToRun)

inline fun <T> trySwallow(failureValue: T,
                          tryToRun: () -> T): T = tryCatch({ failureValue },
                                                           tryToRun)

inline fun <R> tryOrNull(onFailure: (Throwable) -> Unit,
                         tryToRun: () -> R): R? =
    tryCatch({ onFailure(it); null },
             tryToRun)

inline fun <T> tryCatch(tryToRun: () -> T): T? = tryCatch(null,
                                                          tryToRun)

inline fun <T> tryCatch(failureValue: T,
                        tryToRun: () -> T): T =
    tryCatch({ it.printStackTrace(); failureValue },
             tryToRun)
