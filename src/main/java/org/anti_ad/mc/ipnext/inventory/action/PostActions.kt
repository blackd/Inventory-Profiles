...
object PostActions {
  ...
  fun groupInRows(slots: List<ItemStack>, width: Int, height: Int): List<ItemStack> =
    group(slots, width, height, true)

  fun groupInColumns(slots: List<ItemStack>, width: Int, height: Int): List<ItemStack> =
    group(slots, width, height, false)

  private fun group(slots: List<ItemStack>, width: Int, height: Int, inRows: Boolean = false): List<ItemStack> {
    ...
    val calcResult = if (inRows) { // transpose
      GroupInColumnsCalculator(slotCountPairList, height, width).calc()?.map { (itemType, slotIndices) ->
        itemType to slotIndices.asIndicesTranspose(height, width)
      }
    } else {
...
private fun List<Int>.asIndicesTranspose(width: Int, height: Int) =
  map { transposedIndex(width, height, it) }
...
private class GroupInColumnsCalculator(
  ...
  fun calc(): List<Pair<ItemType, List<Int>>>? { // itemType, slotIndices
    ...
    for (columnsCount in 1..width) {
      ...
      val cc = ColumnsCandidate(slotCountPairList, distribute(width, columnsCount), height)
      if (cc.succeeded) {
  ...
  class ColumnsCandidate(
    ...
    val cells: List<Cell> =
      columnWidths.mapIndexed { columnIndex, width -> List(height) { rowIndex -> Cell(width, rowIndex, columnIndex) } }
        .flatten()
    var cellIndex = 0
        ...
        for (i in cellIndex until cells.size) {
          ...
          if (totalRoom >= slotCount) { // enough, check if broken
            if (neededCells > height || cells[i].columnIndex == initColumnIndex) { // no broken
              return cells.subList(cellIndex, cellIndex + neededCells)
    ...
    inner class Cell(val room: Int, val rowIndex: Int, val columnIndex: Int) {
      var occupied = false
      val index
