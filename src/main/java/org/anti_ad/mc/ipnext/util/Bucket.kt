...
interface Bucket<T> {
  ...
  fun contains(element: T, count: Int): Boolean
...
open class MutableBucket<T> protected constructor(innerMap: Map<T, Int>) : Bucket<T> { /* : MutableCollection<T> */
  ...
  fun add(element: T) = add(element, 1)
  fun add(element: T, count: Int): Boolean {
    if (validateEmpty(element) || count <= 0) return false
    innerMap[element] = count(element) + count
    return true
  }

  override fun contains(element: T) = contains(element, 1)
  override fun contains(element: T, count: Int): Boolean {
    return validateEmpty(element) || count(element) >= count
  }

  fun remove(element: T) = remove(element, 1)
  fun remove(element: T, count: Int): Boolean {
    if (validateEmpty(element) || count <= 0) return false
  ...
  fun addAll(another: Bucket<T>): Boolean {
    return another.entrySet.map { add(it.key, it.value) }.any { it }
  }

  override fun containsAll(another: Bucket<T>): Boolean {
    return another.entrySet.all { contains(it.key, it.value) }
  }

  fun removeAll(another: Bucket<T>): Boolean {
    return another.entrySet.map { remove(it.key, it.value) }.any { it }
  }
