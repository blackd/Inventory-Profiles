  ...
  override fun compare(str1: String, str2: String): Int {
    ...
    while (true) {
      ...
      if (index1 >= str1.length || index2 >= str2.length) {
        ...
        return compareWhenEqual(str1, str2, minusCounts1, minusCounts2)
      }
      //endregion
      //region Check digit
      // is '0'..'9', '０'..'９'
      ...
      if (d1 != '\u0000' && d2 != '\u0000') {
        ...
        str1.substring(startIndex1, index1).let { num1 ->
          str2.substring(startIndex2, index2).let { num2 ->
        ...
        textComparator.compare(str1[index1].toString(), str2[index2].toString()).let { result ->
          if (result != 0) return result
        }
        index1++
        index2++
