...
object RandFixedSum {
  ...
  fun randfixedsum(n: Int, max: Int, sum: Int): List<Int> { // TODO use randfixedsum
    ...
    val aRes = randfixedsum(a, max, aSum)
    val bRes = randfixedsum(b, max, sum - aSum)
    return aRes + bRes
  }

  class Constraint(val min: Int, val max: Int)

  fun randfixedsum(nums: List<Constraint>, sum: Int): List<Int> {
    ...
    val a = nums.subList(0, nums.size / 2)
    val b = nums.subList(a.size, nums.size)
    ...
    val aRes = randfixedsum(a, aSum)
    val bRes = randfixedsum(b, sum - aSum)
    return aRes + bRes
  }
...
fun List<Pair<ItemType, List<Int>>>.flatten(resultSize: Int? = null): List<ItemStack> { // itemType, itemCounts
  val a: List<ItemStack> = this.flatMap { (itemType, counts) -> counts.map { ItemStack(itemType, it) } }
...
fun List<ItemStack>.spreadSlot(emptySlot: Int = 0): List<ItemStack> { // random distribute slot
  val stat = this.stat()
  val resultSlotCount = (this.size - emptySlot).coerceIn(stat.totalMinSlotCount, stat.totalMaxSlotCount)
  val groupEntries = stat.groupEntries
  val constraints = groupEntries.map { RandFixedSum.Constraint(it.minSlotCount, it.itemCount) }
  val slotCounts: List<Int> = RandFixedSum.randfixedsum(constraints, resultSlotCount)
  // ok now know slot count for each item type (itemTypes zip slotCounts)
  val itemCountsForEachItemType: List<List<Int>> =
    groupEntries.zip(slotCounts) { entry, slotCount -> distributeMonotonic(entry.itemCount, slotCount) }
...
private inline fun List<ItemStack>.writeEachItemType(itemCounts: (ItemStat.GroupEntry) -> List<Int>): List<ItemStack> {
  ...
  for (entry in stat.groupEntries) {
    itemCounts(entry).zip(entry.slotIndices) { count, index -> // ok now write to result
      result[index] = ItemStack(entry.itemType, count)
    }
  }
  return result
}
...
fun List<ItemStack>.spreadItemCount(): List<ItemStack> { // random distribute count
  return writeEachItemType { entry ->
    val constraint = RandFixedSum.Constraint(1, entry.itemType.maxCount)
    val constraints = List(entry.slotCount) { constraint }
    return@writeEachItemType RandFixedSum.randfixedsum(constraints, entry.itemCount)
  }
}
...
// (9, 6) -> [1, 2, 1, 2, 1, 2]
fun distribute(sum: Int, listSize: Int): List<Int> =
  (0..listSize).map { sum * it / listSize }.zipWithNext { a, b -> b - a }

// (9, 6) -> [2, 2, 2, 1, 1, 1]
fun distributeMonotonic(sum: Int, listSize: Int): List<Int> =
  List(sum % listSize) { sum / listSize + 1 } + List(listSize - sum % listSize) { sum / listSize }

// (30, 6, 7) -> [6, 6, 6, 6, 4, 1, 1]
fun fillOne(sum: Int, max: Int, listSize: Int): List<Int> {
...
fun pack(sum: Int, max: Int): List<Int> {
