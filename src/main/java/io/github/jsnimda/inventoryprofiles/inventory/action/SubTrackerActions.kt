...
import io.github.jsnimda.inventoryprofiles.inventory.data.itemTypes
...
fun MutableSubTracker.moveAllTo(another: MutableSubTracker, skipEmpty: Boolean) {
  val anotherSlots = another.slots
...
private fun List<ItemStack>.sortItems(sortingRule: Rule): List<ItemStack> {
  ...
  return bucket.elementSet.toList().sortedWith(sortingRule)
    .map { itemType -> itemType to pack(bucket.count(itemType), itemType.maxCount) }
    .flatten(this.size)
}
...
private fun MutableItemStack.moveTo(destination: List<MutableItemStack>, skipEmpty: Boolean) {
  if (isEmpty()) return
  destination.forEach {
