@file:Suppress("Reformat")

package io.github.jsnimda.inventoryprofiles.item.rule

import java.text.Collator
import kotlin.reflect.KProperty

object RuleParameters {
  val MAP = mutableMapOf<String, Parameter<*>>()
  val string_compare           by enum<StringCompare>()
  val locale                   by any_string
  val strength                 by enum<Strength>()
  val logical                  by type_boolean
  val match                    by enum<Match>()
  val has_custom_name          by parameterOf(match.argumentType)
  val number_order             by enum<NumberOrder>()
  val sub_comparator_match     by any_comparator
  val sub_comparator_not_match by any_comparator
  val nbt                      by any_nbt
  val allow_extra              by type_boolean
  val require_nbt              by enum<RequireNbt>()
  val tag_name                 by any_tag_name
  val item_name                by any_item_name
  val nbt_path                 by any_nbt_path
  val has_nbt                  by parameterOf(match.argumentType)
  val reverse                  by type_boolean
  val sub_comparator           by any_comparator
}

enum class StringCompare(val comparator: Comparator<in String>?) {
  UNICODE(naturalOrder()),
  IGNORE_CASE(String.CASE_INSENSITIVE_ORDER),
  LOCALE(null)
}

enum class Strength(val value: Int) {
  PRIMARY(Collator.PRIMARY),
  SECONDARY(Collator.SECONDARY),
  TERTIARY(Collator.TERTIARY),
  IDENTICAL(Collator.IDENTICAL)
}

enum class NumberOrder(private val comparator: Comparator<in Double>) : Comparator<Number> {
  ASCENDING(naturalOrder()),
  DESCENDING(reverseOrder());

  override fun compare(num1: Number, num2: Number): Int =
    comparator.compare(num1.toDouble(), num2.toDouble())
}

enum class Match(val multiplier: Int) {
  FIRST(1),
  LAST(-1)
}

enum class RequireNbt {
  REQUIRED,
  NO_NBT,
  NOT_REQUIRED
}


// ============
// Some helper functions for creating parameters
// ============
private class ParameterProvider<T>(private val argumentType: ArgumentType<T>) {
  operator fun provideDelegate(thisRef: RuleParameters, property: KProperty<*>): Parameter<T> =
    Parameter(property.name, argumentType).also { thisRef.MAP[property.name] = it }
}

private operator fun <T> Parameter<T>.getValue(thisRef: RuleParameters, property: KProperty<*>) = this
private fun <T> parameterOf(argumentType: ArgumentType<T>) = ParameterProvider(argumentType)
private val any_string = ParameterProvider(StringArgumentType)
private val type_boolean = ParameterProvider(BooleanArgumentType)
private inline fun <reified T : Enum<T>> enum() = ParameterProvider(EnumArgumentType(T::class.java))
private val any_nbt = ParameterProvider(NbtArgumentType)
private val any_comparator = ParameterProvider(ComparatorArgumentType)
private val any_tag_name = ParameterProvider(TagNameArgumentType)
private val any_item_name = ParameterProvider(ItemNameArgumentType)
private val any_nbt_path = ParameterProvider(NbtPathArgumentType)
