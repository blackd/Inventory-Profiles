...
inline fun <T> compareByMatchSeparate(
  ...
  return compareByMatchSeparate(value1, value2, matchBy(value1), matchBy(value2), match, matchCompare, notMatchCompare)
}
...
class ByNbtRule : NativeRule() {
  ...
  val dummies = listOf(dummyStringRule, dummyNumberRule)

  init {
    arguments.apply {
      defineParameter(nbt_path)
      defineParameter(not_found, Match.LAST)
  ...
  fun compareByNbtPath(itemType1: ItemType, itemType2: ItemType): Int {
    initializeDummyArguments()
    return ByNbtPathComparator(itemType1, itemType2).compare()
  }

  inner class ByNbtPathComparator(val itemType1: ItemType, val itemType2: ItemType) {
    fun compare(): Int {
    ...
    fun compareTag(tag1: NbtUtils.WrappedTag, tag2: NbtUtils.WrappedTag): Int {
      return when {
      ...
      } ?: compareAsString(tag1, tag2)
    }

    fun compareAsString(tag1: NbtUtils.WrappedTag, tag2: NbtUtils.WrappedTag): Int {
      return dummyStringRule.compareString(tag1.asString, tag2.asString)
    }

    fun compareAsNumber(tag1: NbtUtils.WrappedTag, tag2: NbtUtils.WrappedTag): Int {
      return dummyNumberRule.compareNumber(tag1.asNumber, tag2.asNumber)
    }

    fun compareAsCompound(tag1: NbtUtils.WrappedTag, tag2: NbtUtils.WrappedTag): Int {
      return NbtUtils.compareNbt(tag1.asCompound, tag2.asCompound)
    }

    fun compareAsList(tag1: NbtUtils.WrappedTag, tag2: NbtUtils.WrappedTag): Int {
