...
public class BuiltInMethods {
  ...
  public static int custom_name(VirtualItemType a, VirtualItemType b, StringCompareMethod m) {
    int aHas = ItemUtils.getItemStack(a).hasDisplayName() ? 1 : 0;
    int bHas = ItemUtils.getItemStack(b).hasDisplayName() ? 1 : 0;
    if (aHas == 1 && bHas == 1) {
      String aStr = ItemUtils.getItemStack(a).getDisplayName().getString();
      String bStr = ItemUtils.getItemStack(b).getDisplayName().getString();
  ...
  public static double getEnchantmentsScore(VirtualItemType type) {
    ...
    for (Entry<Enchantment, Integer> e : enc.entrySet()) {
      if (!e.getKey().isCurse()) { // cursed enchantments +0 scores
  ...
  public static int raw_id(VirtualItemType a, VirtualItemType b) {
    int aInt = Item.getIdFromItem(a.item);
    int bInt = Item.getIdFromItem(b.item);
    return aInt - bInt;
  }
  ...
  public static int has_potion_effects(VirtualItemType a, VirtualItemType b) {
    int aHas = PotionUtils.getEffectsFromTag(a.tag).isEmpty() ? 0 : 1;
    int bHas = PotionUtils.getEffectsFromTag(b.tag).isEmpty() ? 0 : 1;
    return bHas - aHas;
  }

  public static int has_custom_potion_effects(VirtualItemType a, VirtualItemType b) {
    int aHas = PotionUtils.getFullEffectsFromTag(a.tag).isEmpty() ? 0 : 1;
    int bHas = PotionUtils.getFullEffectsFromTag(b.tag).isEmpty() ? 0 : 1;
    return bHas - aHas;
  }
  ...
  public static int potion_effects(VirtualItemType a, VirtualItemType b) {
    List<EffectInstance> aList = PotionUtils.getEffectsFromTag(a.tag);
    List<EffectInstance> bList = PotionUtils.getEffectsFromTag(b.tag);
