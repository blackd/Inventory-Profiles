...
public class ContainerUtils {
  ...
  public static int getRemainingRoom(@Nullable Slot slot, @Nullable ItemStack forItem) {
    ...
    if (!slot.isItemValid(forItem)) return 0;
    if (!slot.getHasStack()) return slot.getItemStackLimit(forItem);
    ItemStack slotItem = slot.getStack();
    if (!areItemsEqual(slotItem, forItem)) return 0;
    int maxAmount = Math.min(slot.getItemStackLimit(slotItem), slotItem.getMaxStackSize());
