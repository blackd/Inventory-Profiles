...
public class ContainerInfo {
    ...
    public ListMultimap<IInventory, Slot> slotsMap = ArrayListMultimap.create();  // slots in container grouped by their inventory (if any)
    ...
    private void translatePlayer(Container asContainer) {
      ...
      for (Slot s : asContainer.inventorySlots) { // assumed in order of slot.id
        if (!(s.inventory instanceof PlayerInventory)) continue;
        int invSlot = (s).getSlotIndex();
        if (invSlot >= 0 && invSlot < 9) {
          playerHotbarSlots.add(s);
          if (((PlayerInventory)s.inventory).currentItem == invSlot) {
    ...
    private void translateNonPlayer() {
      for (Slot s : container.inventorySlots) { // (need to test hidden slots?)
        if (s.inventory instanceof PlayerInventory) continue;
      ...
      for (Slot s : container.inventorySlots) {
        if (s.inventory instanceof PlayerInventory) continue;
        if (s instanceof MerchantResultSlot) {
          traderOutputSlots.add(s);
        }
        if (s.inventory instanceof CraftingInventory) {
          craftingSlots.add(s);
        } else if (s.inventory instanceof CraftResultInventory) {
          craftingResultSlots.add(s);
        } else if (s.inventory instanceof MerchantInventory) {
          traderSlots.add(s);
          if (!(s instanceof MerchantResultSlot)) {
            traderInputSlots.add(s);
          }
        } else if (!(s instanceof MerchantResultSlot)) {
