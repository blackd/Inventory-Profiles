...
inline fun <T> T.runIf(condition: Boolean,
                       block: T.() -> T): T = if (condition) run(block) else this

inline fun <T> T.letIf(condition: Boolean,
                       block: (T) -> T): T = if (condition) let(block) else this

inline fun <T> T.applyIf(condition: Boolean,
                         block: T.() -> Unit): T = if (condition) apply(block) else this

inline fun <T> T.alsoIf(condition: Boolean,
                        block: (T) -> Unit): T = if (condition) also(block) else this
...
inline fun <T> T.letIf(condition: (T) -> Boolean,
                       block: (T) -> T): T = if (condition(this)) let(block) else this

inline fun <T> T.applyIf(condition: T.() -> Boolean,
                         block: T.() -> Unit): T = if (condition()) apply(block) else this

inline fun <T> T.alsoIf(condition: (T) -> Boolean,
                        block: (T) -> Unit): T = if (condition(this)) also(block) else this
...
class AsComparable<T>(val value: T,
                      val comparator: Comparator<T>) : Comparable<AsComparable<T>> {
    override fun compareTo(other: AsComparable<T>): Int = comparator.compare(value,
                                                                             other.value)
}

fun <T> T.asComparable(comparator: Comparator<T>) = AsComparable(this,
                                                                 comparator)

fun <T : Comparable<T>> List<T>.asComparable() = asComparable(Comparator(::compareList))
fun <T : Comparable<T>> List<T>.compareTo(other: List<T>): Int = compareList(this,
                                                                             other)

private fun <T : Comparable<T>> compareList(a: List<T>,
                                            b: List<T>): Int {
    for (i in 0 until minOf(a.size,
                            b.size)) {
...
fun <T> List<Comparator<T>>.compare(a: T,
                                    b: T): Int {
    forEach {
        it.compare(a,
                   b).let { result -> if (result != 0) return result }
    }
    return 0
}
